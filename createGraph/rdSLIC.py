...
from .CreatHStructure import createH
...
class SLIC(object):
    def __init__(self, HSI, labels, n_segments=1000, compactness=2,
    ...
    def get_Q_and_S_and_Segments(self):
        ...
        np.save("segments_100",segments)
        print("save segments over")
        ...
        # plt.show()
        # ##################################### 显示超像素图片 ######################################
        segments_spa = segments
        segments = np.reshape(segments, [-1])           
        S = np.zeros([superpixel_count, d], dtype=np.float32)
        segment_center = np.zeros([superpixel_count, 2], dtype=np.float32)
        ...
        for i in range(superpixel_count):
            idx = np.where(segments_spa == i)
            segment_indices_row, segment_indices_col = idx
            segment_center_row = np.mean(segment_indices_row)
            segment_center_col = np.mean(segment_indices_col)
            segment_center[i][0] = segment_center_row
            segment_center[i][1] = segment_center_col
        #####构造超图
        AH = createH(S, segment_center,segments_spa)
        # AH = []
        self.S = S
        self.Q = Q

        return Q, S, self.segments, AH
...
class LDA_SLIC(object):
    ...
    def SLIC_Process(self, img, scale=25):
        ...
        myslic=SLIC(img,n_segments=n_segments_init,labels=self.labes, compactness=0.5,sigma=1, min_size_factor=0.1, max_size_factor=2)
        Q, S, Segments,AH = myslic.get_Q_and_S_and_Segments()
        A=myslic.get_A(sigma=10)
        return Q, S, AH, Segments
